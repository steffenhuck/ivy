...
These notes are in their second edition, and the reason is a playtest. A reviewer reported reaching the top of the scheme by year five with the least interesting strategy the game admits---spend the full budget, split it equally, repeat---and holding the summit without further thought. The harness confirmed the report and sharpened it: scripted, the strategy topped the scheme in forty-three per cent of seeds, more often than the sharp player, while requiring none of the sharp player's judgement. Section~\ref{sec:second} records the diagnosis and the repair. The short version is that the rival colleges were hoarding cash the scheme gives them no reason to hold, and that money at the end of the game was worth nothing to anybody; both were failures of institutional imagination rather than of code, which is why they earn a section rather than a footnote.

The notes proceed as follows. Section~\ref{sec:env} sets out the environment common to both worlds. Sections~\ref{sec:market} and~\ref{sec:scheme} specify the two institutions. Section~\ref{sec:second} reports the playtest and the second edition's four repairs. Section~\ref{sec:calib} reports the calibration and a comparison of the two games' difficulty. Section~\ref{sec:disc} discusses what the comparison teaches.
...
\paragraph{The ledger.} Fees are paid up front. With fee income $F_t$, the year's bills $C_t$---seat costs as above plus, in the second edition, any scholarship fund---and investment $I_t$,
\[
...
\paragraph{Merit scholarships (second edition).} In either world a college may endow, per field and per year, a scholarship fund $G \ge 0$, capped at 20. The fund is committed at the admissions desk and spent in full whether anyone comes; endowed stipends are advertised, not refunded. Its effect runs through student preferences: an applicant with score $s \ge 70$ values the advertising college $2.2\sqrt{G}$ utility points higher---in the market's choice among offers and in the scheme's preference lists identically. The concavity is deliberate, and the merit bar makes the instrument what merit aid is: useless for buying bodies, useful for tipping the bright student who was almost persuaded already. Section~\ref{sec:second} reports the probe evidence that the fund is a lever rather than a freebie.

\paragraph{The post (second edition).} Beyond the shocks, the player---and only the player---receives dilemmas: roughly a dozen distinct cards over the library, offered with probability~0.3 a year from year two, each at most once per game. A card names a situation (a rival's professor is approachable; a donor offers money with conditions attached; an insurer offers a rider against the storm season), prints its price and its odds, and demands a decision before the year proceeds. Cards are drawn on the same separate random stream as the shocks, with all draws---including the success roll---consumed unconditionally, so no decision the player takes can perturb the applicant stream or the rivals' luck. An unanswered card resolves as a decline, which for the insurance-shaped cards means the gamble is taken by default: an unattended college buys no cover. Card prices are set against the investment technology $\gamma\sqrt{I}$, so that most cards are close to fair at a heavy investor's margins, poaching carries a zero-sum premium, and one card is priced deliberately below the technology. The reader who checks the arithmetic keeps their money.

\paragraph{The Founders' Reckoning (second edition).} The game's final table converts remaining endowments into league points: each college is credited $\min\!\big(\max(0, E)/15,\; 8\big)$ points, announced in the rules from year one and identical in both worlds. The design intent and the honest economics are worth separating. The intent, proposed by the playtest, was a spend-now-versus-save choice. The economics is that for the player the credit is dominated at any realistic scale: a final-round pound invested buys more quality than a pound reckoned until investment approaches five hundred per field, which no solvent college reaches. The Reckoning's bite lies elsewhere. The rival incumbents end typical games with hoards north of a thousand---capped, those hoards are now worth up to eight points against a challenger, in both worlds. The Reckoning is a difficulty device wearing a savings product's clothes, and these notes say so plainly.
...
Two theoretical properties of this design do the game's work. The mechanism is strategy-proof for students (Roth, 1982)---fitting, since the students are honest automata---but no stable mechanism is strategy-proof for the colleges (Roth, 1985), and this one is manipulable in a specific, profitable direction: by capacities (S\"onmez, 1997). A department that reports fewer seats than its demand holds only its best proposers; its intake calibre rises, the $\kappa$ term feeds its teaching, and better teaching attracts better proposers. Since the rent is owed on the seats either way, the manipulation here is undiluted by economy: shrinking the report buys calibre at the price of forgone fees, exactly as the theory has it. In the first edition the scripted sharp player's mid-game was precisely this manipulation. In the second edition it no longer is, and the reason is instructive: against rivals who spend their guaranteed income rationally (Section~\ref{sec:second}), the fees a small quota forgoes are worth more than the calibre it buys, and the calibrated best play is volume-first---every seat declared, the threshold doing the selecting. The manipulation remains available, remains undiluted, and remains, at the margin, a mistake. Whether a manipulation pays is a statement about one's rivals as much as about the mechanism; a player who discovers both halves has discovered a theorem and its comparative statics.
...
\section{The playtest, and the second edition}\label{sec:second}

The reviewer's strategy deserves to be stated in full, because its full statement is an indictment: declare every seat, keep a workmanlike threshold, spend the entire budget every year, split it equally across the four qualities. Scripted and run over the five hundred calibration seeds of the first edition, it topped the scheme in 43.2 per cent of them---nine points more often than the sharp player---while in the market the same strategy went bankrupt in 57 per cent of seeds. The market was policing full-throttle spending; the scheme was rewarding it.

The diagnosis has three parts, in ascending order of embarrassment. First, there is no downside in the scheme: deferred acceptance never over-fills, so the only ruin available is self-inflicted. Second, the quality technology is symmetric and concave, so splitting equally is close to optimal allocation and no skill is exercised by it. Third---the part that matters---the rival colleges were carrying precautionary cash floats and conservative spending ceilings calibrated against market risk, in a world whose clearing house guarantees their revenue. Precautionary saving without uncertainty is not caution but confusion, and the reviewer's strategy was simply the only agent in the room spending like a rational actor. The exploit was not a hole in the game; it was a correct answer the designers had failed to anticipate.

The repair is a package of four levers, each calibrated one at a time against the harness. The first is the rationality correction: scheme-world rivals now hold only a thin float, and their plan ceilings loosen by the factor~1.35---a correction, not a handicap, for the reason just given. The second is the Founders' Reckoning of Section~\ref{sec:env}, which hands the incumbents' hoards a capped terminal value. The third is the scholarship instrument, whose probe evidence is worth one sentence each way: a probe running heavy standing pots on a sensible budget goes bankrupt in 100 per cent of seeds in both worlds, so the fund cannot be spammed; and its measured effect is asymmetric in exactly the way merit aid should be---in the market a modest pot lifts a bottom college's intake calibre by about two score points (the contested bright applicant is choosing among affordable houses of similar appeal), while in the scheme it moves nothing until the fighting colleges are within a stipend's width of one another (the bright student's next-best option is simply better than you, pot or no pot). The fourth is the card deck, whose accept-everything probe does no better than sensible play once its extra bankruptcies are counted.

The levers interact with best play, and the sharp player had to be re-derived; Section~\ref{sec:scheme} records the headline, the volume-first turn. What survived calibration is this: the sharp player tops roughly a third of seeds in both worlds, as the original brief demanded; the reviewer's strategy tops the scheme in 9.8 per cent of seeds and reaches the summit by year five and holds it in none of five hundred; and the harness now carries that strategy permanently, as the row \emph{rustam} in every future report. A game owes its best players its vigilance, and the cheapest way to honour a critic is to name the regression test after him.

\section{Scripted play and calibration}\label{sec:calib}

Each world is calibrated with four scripted players run from the worst starting position over seeds $1$--$500$. The first three tiers are constructed from the same recipes in both worlds. \emph{Naive} makes static mid-range choices and spreads investment evenly. \emph{Sensible} tracks realised demand with its instruments, maintains its qualities against decay, and keeps a reserve. \emph{Sharp} exploits the institution: in the market, revenue hill-climbing on the captive segment, threshold-farming of the intake effect, yield forecasting and top-end rationing; in the scheme, volume-first admissions with the threshold as the selectivity instrument; in both, scholarship pots in the summit fight and card purchases whenever liquidity and the printed odds allow---every card but the consultancy's. The fourth row, \emph{rustam}, is the playtest strategy of Section~\ref{sec:second}, unchanged and permanent. Table~\ref{tab:calib} reports outcomes; Table~\ref{tab:diff} reports derived difficulty metrics on the same seeds.
...
scheme & naive    & 0.43 & 2.0  & 33.8 & 66.2 & 65.6 \\
       ...
       & sharp    & 2.18 & 34.8 & 91.8 & 8.2  & 1.2 \\
       & rustam   & 1.09 & 9.8  & 52.0 & 48.0 & 10.6 \\
\bottomrule
\end{tabular}
\caption{Calibration over 500 seeded histories per cell, player at the worst start (rank 4). Score $=$ initial rank $-$ final rank; ``improves'' is score $\ge 1$. Second-edition engine throughout; the first edition's numbers appear in Section~\ref{sec:second} where they differ instructively.}
\label{tab:calib}
\end{table}
...
With that caveat, four regularities survive. First, the rent bills the sleeping in both worlds, but only the market executes them: static play goes bankrupt in effectively every market seed (99.6 per cent), while under the scheme a third of static colleges survive the early grind. What has changed since the first edition is the fate of the survivors: they used to top the table in 18 per cent of seeds, and now do so in 2.0---rationally-spending rivals no longer leave the summit to the idle. The institutional lesson stands where it belongs: where there is no price, there is no selection, and the clearing house keeps the asleep alive; it just no longer crowns them. Second, mastery pays everywhere and calm is scheme-specific: sharp play reaches the top in a third of seeds in both worlds (32.0 and 34.8 per cent), but with much less noise under the scheme (s.d.\ 0.80 against 1.19, bankruptcy 1.2 against 9.4)---deferred acceptance abolishes the market's yield lottery, and what remains of sharp risk in the market is the accepted-offer rule meeting a quality crossing. Third, the rustam rows measure what full-throttle spending is worth under each institution: in the market it is a coin toss against ruin (bankrupt in 61 per cent of seeds), under the scheme it is safe, mediocre, and no longer sovereign (tops 9.8 per cent, reaches the summit by year five and holds it in none). Fourth, prudence buys survival, not distinction, and more than ever: sensible improves in 30 and 43 per cent of seeds but tops in 2.0 and 0.0---the Reckoning hands the hoarding incumbents up to eight free points, and the reserve that keeps a sensible college solvent is precisely what it declines to spend on catching them.
...
What the comparison does teach is structural, and it survives the caveat. Remove the price instrument, and competition does not disappear---it migrates entirely into quality, where incumbents with hoarded endowments hold the high ground; the resulting reaction functions deter the half-hearted challenges that the market, indifferently, lets through, yet they cannot tell a compounding gambler from a compounding talent. Guarantee demand through a clearing house, and the fixed cost of seats stops selecting: the market forecloses on the passive, the scheme merely invoices them---and whether the invoiced ever reach the top turns out to depend not on the mechanism but on the conduct of their rivals, which is the second edition's whole story in one clause. And the endowment experiment deserves its own line: the same war chest that finances patient learning under price competition finances indiscriminate expansion under guaranteed demand---which is why the two worlds start with different money and identical published tables. What capital is worth depends on the institution it is spent in.

The playtest adds a lesson about design itself. The first edition's rivals were prudent everywhere because prudence had been calibrated where it was needed---the market---and exported, unexamined, to a world that punishes it. The exploit that resulted was not a bug in any line of code; every component behaved as specified. It was an equilibrium the designers had not solved for, found within days by a player who had. The repair, accordingly, was not a patch but a rationality correction, and the test that guards it is the exploiting strategy itself, named for its discoverer and run on every build. Designed economies fail the way real ones do: through agents behaving reasonably in ways the regulator did not model.
...
--- & 70; 2.2; 20 & scholarship merit bar; potency ($2.2\sqrt{G}$); fund cap & both \\
