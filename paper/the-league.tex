...
\paragraph{The ledger.} Fees are paid up front. With fee income $F_t$, the year's bills $C_t$---seat costs as above plus, in the second edition, the stipend bill---and investment $I_t$,
\[
...
\paragraph{Merit scholarships (second edition; per-head form after further playtest discussion).} In either world a college sets, per field and per year, a stipend $\sigma \in [0, 20]$ per student and a qualifying bar $\beta \in [40, 95]$ in school score. Every enrolling student with $s \ge \beta$ is owed the stipend, so the bill is $\sigma$ times the qualifying intake---known only after clearing, part of $C_t$, and therefore able to bankrupt: a generous stipend at a low bar is a road to ruin, not a bargain, and the calibration probes confirm it (Section~\ref{sec:second}). An earlier draft modelled the instrument as a committed fund whose pull was concave in its size; that version asked applicants to value a pot whose division among unknown rivals they could not compute, and it was retired for exactly that reason. The per-head form asks a student to know only her own offer.
...
The repair is a package of four levers, each calibrated one at a time against the harness. The first is the rationality correction: scheme-world rivals now hold only a thin float, and their plan ceilings loosen by the factor~1.35---a correction, not a handicap, for the reason just given. The second is the Founders' Reckoning of Section~\ref{sec:env}, which hands the incumbents' hoards a capped terminal value. The third is the scholarship instrument of Section~\ref{sec:env}, in its per-head form, whose probe evidence is worth one sentence each way: a probe promising a stipend of 10 at a bar of 50 on a sensible budget---nearly every matriculant qualifies---goes bankrupt in effectively every seed in both worlds, so the instrument cannot be spammed, and the new route to ruin it opens is real; while used as designed, at a high bar in a summit fight, its effect is what merit aid's should be---decisive only when the fighting colleges sit within a stipend's width of one another in the students' eyes. The fourth is the card deck, whose accept-everything probe does no better than sensible play once its extra bankruptcies are counted.
