...
lines.push('  3. Merit stipends (both worlds, third revision of the instrument):');
lines.push('     per field, a PER-HEAD stipend (cap 20) and a qualifying score');
lines.push('     bar (40-95, default 70) set by the administrator. Every');
lines.push('     qualifying matriculant is owed the stipend; the bill settles');
lines.push('     with the intake and is part of C, so it can bankrupt. Channel');
lines.push('     by world, on principle: in the market the stipend counts toward');
lines.push('     the fee for qualifiers (affordability: fee - sch <= b; choice');
lines.push('     among offers stays quality-only, as fees themselves never sway');
lines.push('     choice) -- merit aid as targeted price discrimination; in the');
lines.push('     scheme a qualifier ranks the college schAlpha*sch (=1.0/k)');
lines.push('     higher in DA -- a side payment, the only price competition the');
lines.push('     regulator has left. The scholar probe (stipend 10 at bar 50 on');
lines.push('     a sensible budget) is bankrupt in ~100% of seeds: the match');
lines.push('     takes a generous promise at its word.');
...
lines.push('          instrument, high-bar stipends in the summit fight, cards by');
